...
class Customer extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'lastname' => array('type' => self::TYPE_STRING, 'required' => false, 'size' => 65),
            'firstname' => array('type' => self::TYPE_STRING, 'required' => false, 'size' => 65),
            'email' => array('type' => self::TYPE_STRING, 'validate' => 'isEmail', 'required' => false, 'size' => 255,'allow_null' =>true),
            
            'name' => array('type' => self::TYPE_STRING, 'validate' => 'isCustomerName', 'required' => false, 'size' => 255),    
             /** register phone number **/
            'mobile_no' => array('type' => self::TYPE_STRING,'validate' => 'isPhoneNumber', 'required' => false, 'size' => 20),
    ...
    public function getByMobileLogin($mobile_no, $plaintextPassword = null, $ignoreGuest = true)
    {
        if (!Validate::isPhoneNumber($mobile_no) || ($plaintextPassword && !Validate::isPasswd($plaintextPassword))) {
            die(Tools::displayError());
        }
    ...
    public static function customerExistsByMobile($mobile_no, $returnId = false, $ignoreGuest = true)
    {
        ...
        SELECT `id_customer`
        FROM `' . _DB_PREFIX_ . 'customer`
        WHERE `mobile_no` = \'' . pSQL($mobile_no) . '\'
        ' . Shop::addSqlRestriction(Shop::SHARE_CUSTOMER) . '
        ' . ($ignoreGuest ? ' AND `is_guest` = 0' : ''));
    ...
    public static function searchByName($query, $limit = null)
    {
        ...
        $research_fields = array('id_customer', 'name', 'mobile_no', 'email');
