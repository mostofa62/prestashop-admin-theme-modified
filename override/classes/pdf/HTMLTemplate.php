...
abstract class HTMLTemplateCore
{
    ...
    protected function getShopAddress()
    {
        ...
        if (isset($shop_address_obj) && $shop_address_obj instanceof Address) {
            ...
            $shop_address = $this->generateAddress($shop_address_obj);
        }

        return $shop_address;
    }
    //mostofa
    protected function generateAddress(Address $address)
    {
        
        $country = Country::getNameById((int) Context::getContext()->language->id,(int) $address->id_country);

        $data=$this->shop->name.", ";
        $data.=isset($address->address1)?$address->address1.", ":null;
        $data.=isset($address->city)?$address->city.", ":null;
        $data.=isset($country)?$country.".<br/>":null;
        if(isset($address->phone_mobile) &&  isset($address->phone)){
            $data.=$address->phone_mobile.", ".$address->phone.".";
