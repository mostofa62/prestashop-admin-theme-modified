...
final class LinkBlockQueryBuilder extends AbstractDoctrineQueryBuilder
{
    ...
    public function getSearchQueryBuilder(SearchCriteriaInterface $searchCriteria = null)
    {
        ...
        $qb->select('lb.id_banner, lb.title')->orderBy(
    ...
    private function getQueryBuilder(array $filters)
    {
        $qb = $this->connection
            ->createQueryBuilder()
            ->from($this->dbPrefix . 'arkylus_bannermanager', 'lb');
          /* 
         
        foreach ($filters as $name => $value) {
            if ('title' === $name) {
                $qb
                    ->andWhere("lb.title = :$name")
            ...
            $qb
                ->andWhere(sprintf('lb.%s LIKE :%s', $name, $name))
                ->setParameter($name, '%' . $value . '%')
            ;
        }*/
